...
export default function EventDetailPage() {
  ...
  const rawItems = eventsData.items as EventItem[];
  const event = rawItems.find((e) => e.id === eventId) ?? null;
  ...
  let filteredFiles: typeof event extends null ? never[] : NonNullable<typeof event>["files"] = [];
  if (event) {
    const searchLower = search.toLowerCase();
    const isSearching = searchLower.length > 0;
    const isAccessActive = accessFilter !== "All";
    filteredFiles = event.files.filter((file) => {
  ...
  if (!event) {
    return (
      <PageLayout id="page-event-detail">
  ...
  const getAccess = (id: string) => accessOverrides[id] ?? event?.files.find((f) => f.id === id)?.access ?? "Internal";
  const menuItemAccess = menuItemId ? getAccess(menuItemId) : null;
  const isMenuItemPublic = menuItemAccess === "Public";
  const isEventPublic = (accessOverrides[event?.id ?? ""] ?? event?.access) === "Public";
  const publishTargetAccess = publishTargetId
    ? (publishTargetId === event?.id ? (accessOverrides[event?.id ?? ""] ?? event?.access) : getAccess(publishTargetId))
    : null;
  ...
  const breadcrumbItems = [
    ...
    { id: "current", label: event.name, isCurrent: true },
  ];
        ...
        <PageHeader
          breadcrumbs={
            <OverflowBreadcrumbs items={breadcrumbItems}>
              {(item) => {
                ...
                if (item.id === "library") {
                  return <Link key={item.id} underline="hover" variant="body1" sx={{ cursor: "pointer" }} onClick={() => navigate("/library/events")}>{item.label}</Link>;
                }
                return <Link key={item.id} underline="hover" variant="body1" sx={{ cursor: "pointer" }} onClick={() => navigate("/library/events")}>{item.label}</Link>;
              }}
            </OverflowBreadcrumbs>
          ...
          pageSubtitle={
            <Stack direction="row" spacing={1} alignItems="center">
              <StatusChip label={accessOverrides[event.id] ?? event.access} />
            </Stack>
          }
          moreButton={
            <IconButton
